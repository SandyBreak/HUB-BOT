...
@router.message(F.text.not_in(['/start', '/control', '/help']))
async def catch_message(message: Message, bot: Bot) -> None:
    ...
    if not(id_topic_chat):
        ...
        if (message.chat.id != SUPER_GROUP_ID):
            try:
                ...
                with suppress(TelegramBadRequest):
                    try:
                        ...
                        logging.warning('Topic name is current')
            ...
            user_chat_id = await mongodb_interface.find_user_id(message_thread_id)
            """
