...
class Interaction:
	...
	async def get_super_group_id(self) -> Union[int, str, float, dict]:
		'''
		...
		document = await self.__current_data.find_one({'_id': ObjectId('66bf2af6825485184a414d78')})
		if document:
			super_group_id = document.get("super_group_id")
	...
	async def find_user_message_thread_id(self, message_thread_id: int) -> str:
		'''
		Инициализация пользователя в базе данных
		'''
		document = await self.find_data({'_id': ObjectId('66bf2af6825485184a414d78')})
